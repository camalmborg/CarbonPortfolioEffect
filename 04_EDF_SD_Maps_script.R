...
make_region_maps <- function(var, vec){
  ...
  map_palette = c('#fff7ec','#fee8c8','#fdd49e','#fdbb84','#fc8d59','#ef6548','#d7301f','#b30000','#7f0000','#400000')
  min_value = min(vec$crop_Tot_SD)
  max_value = max(vec$crop_ensVar_SD)
  #breaks = c(0, 11^seq(log10(min_value), log10(max_value), length.out = length(map_palette)))
  #breaks = c(seq(from = 0, to = 1200, length.out = length(map_palette)))
  breaks <- c(0, 10^seq(0, 4, length.out = length(map_palette)))
  ...
  map1 <- ggplot(sf_vec) +
    ...
    coord_sf(xlim <- c(ext[1], ext[2]),
             ylim <- c(ext[3], ext[4])) +
    scale_x_continuous(breaks = round(seq(ext[1], ext[2], length = 4), 0)) +
    scale_y_continuous(breaks = round(seq(ext[3], ext[4], length = 4), 0)) +
    labs(title = "Naive") +
    theme_minimal() +
  ...
  # make together:
  compare_maps <- map2 + map1 + plot_layout(ncol = 1, guides = "collect")
  return(compare_maps)
}
